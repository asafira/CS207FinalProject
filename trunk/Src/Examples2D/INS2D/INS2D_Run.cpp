...
//return;
  ...
  INSPressureSetUp2D();       // Build pressure matrix and boundary forcing (IPDG)
  NDG_garbage_collect();      // recover memory from registry

  INSViscousSetUp2D();        // Build viscous matrix and boundary forcing (IPDG)
  NDG_garbage_collect();      // recover memory from registry
  ...
  for (tstep=1; tstep<=Nsteps; ++tstep)
  {
    ...
    //INSAdvection2D;       // curved version

    INSPressure2D();        // compute pressure PR and intermediate UxTT, UyTT
    //INSPressure2D;        // curved version

    INSViscous2D();         // compute viscous solves and update velocity
