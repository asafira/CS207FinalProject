...
//---------------------------------------------------------
bool NDG2D::MeshReaderGambit2D(const string& fname)
//---------------------------------------------------------
{
  // function [Nv, VX, VY, K, EToV] = MeshReaderGambit2D(FileName)
  ...
  if (! file_exists(FileName)) {
    umWARNING("NDG2D::MeshReaderGambit2D()", "cannot find file %s", fname.c_str());
    return false;
  }
  ...
  if (!is) {
    umWARNING("NDG2D::MeshReaderGambit2D()", "failed to load file %s", fname.c_str());
    return false;
  }
  ...
  //---------------------------------------------
  if (Nsd<2 || Nsd>3) {
    umWARNING("NDG2D::load_mesh()", "Number of space dimensions was %d?", Nsd);
    return false;
  }
...
//double gepsln = 3.0;      // [0:6]  Map colors to AMR cell status
  ...
  //materialVals(i) = i;
    ...
    if (! strstr(buf, "ENDOFSECTION")) {
      umWARNING("NDG2D::load_mesh()", "Expected [ENDOFSECTION]");
      return false;
    }
    is.getline(buf, BUFSIZ);  // Skip "ELEMENT GROUP 1.3.0"
  }
  ...
  for (i=1; i<=Nbcs; ++i)
  {
    ...
    if (! load_BF_group(is, buf)) {
      umWARNING("NDG2D::load_mesh()", "Error loading boundary face group: %s\n", buf);
      return false;
    }
...
//---------------------------------------------------------
bool NDG2D::load_BF_group(istream& is, char* buf)
//---------------------------------------------------------
{
  ...
  if      (match_BC(name, "Infl"))  bcflag = BC_In;
    ...
    umWARNING("NDG2D::load_BF_group()", "Unexpected boundary condition group, '%s' ", name.c_str());
    return false;
  }
